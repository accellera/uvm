...
class ubus_env extends uvm_component;
    ...
    virtual task run();
        ...
        uvm_scoreboard end_to_end_sb;
        
        super.run();
        assert(uvm_config_db#(uvm_scoreboard)::get(this,"","scoreboard",end_to_end_sb));
        forever begin
            @(posedge vif.sig_reset);
            `uvm_info("RESET","CAUSING rerun()",UVM_NONE)
            // dont end the test during reset 
            run_phase.raise_objection(this,"do-not-end-during reset");        
            rerun();
            end_to_end_sb.rerun();
            run_phase.drop_objection(this,"do-not-end-during reset");
        end 
    endtask
