...
if($? != 0) {
  $post_test = "output gold file mismatched";
  system("rm -f $newlog") unless $opt_d;
  return 1;
}

system("diff $path/tr_db.log.au $path/tr_db.log > $path/tr_db.log.df");
if($? != 0) {
  $post_test = "tr_db.log gold file mismatched";
  system("rm -f $path/tr_db.log") unless $opt_d;
  return 1;
}

$post_test = "gold file matched";
system("rm -f $path/output.df");
system("rm -f $path/tr_db.df");
system("rm -f $newlog") unless $opt_d;
system("rm -f $path/tr_db.log") unless $opt_d;
return 0;
