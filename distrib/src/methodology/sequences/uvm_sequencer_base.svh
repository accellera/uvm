...
class uvm_sequencer_base extends uvm_component;
  ...
  // Variable: arb_sequence_q
  //
  // A queue containing descriptors for each active sequence running on this
  // sequencer.
  //
  protected seq_req_info       arb_sequence_q[$];
  ...
  extern function void set_phase_seq (uvm_phase_imp      phase,
                                      uvm_object_wrapper imp,
  ...
  extern protected function int    m_get_seq_item_priority(seq_req_info seq_q_entry);
...
function void uvm_sequencer_base::phase_started(uvm_phase_schedule phase);
  ...
  if(!$cast(seq , f.create_object_by_type(w, get_full_name(), w.get_type_name()))) begin
    `uvm_warning("BDFCT", {"Default phase object for ", phase.get_name(), 
                           "_ph did not cast to a sequence type"})
  end

  if(seq == null) begin
    `uvm_info("PHASESEQ",
         {"No default phase sequence for phase ", phase.get_name()}, UVM_FULL)
  end
  else begin
    `uvm_info("PHASESEQ", {"Starting default phase sequence ", w.get_type_name(),
    ...
    if (!seq.randomize()) begin
      `uvm_warning("STRDEFSEQ", {"Failed to randomize default sequence ",
                      w.get_type_name()," for phase ", phase.get_name()});
    end
    else begin
...
function void uvm_sequencer_base::phase_ended(uvm_phase_schedule phase);
  ...
  if(m_processes.exists(phase)) begin
    ...
    begin
      ...
      if(m_phase_sequences.exists(phase)) begin
        seq_ptr = m_find_sequence(m_phase_sequences[phase].get_sequence_id());
        if(seq_ptr != null)
...
function void uvm_sequencer_base::set_phase_seq (uvm_phase_imp phase, uvm_object_wrapper imp,
    uvm_thread_mode_t thread_mode=UVM_PHASE_MODE_DEFAULT); 
...
function int uvm_sequencer_base::m_get_seq_item_priority(seq_req_info seq_q_entry);
