...
class uvm_comparer;
  ...
  virtual function bit compare_field (string name, 
    ...
    if((lhs & mask) !== (rhs & mask)) begin
      uvm_object::m_sc.scope.set_arg(name);
      case (radix)
        UVM_BIN: begin
  ...
  virtual function bit compare_object (string name,
    ...
    uvm_object::m_sc.scope.down(name);
    compare_object = lhs.compare(rhs, this);
    uvm_object::m_sc.scope.up();
  ...
  function void print_msg (string msg);
    ...
    if(result <= show_max) begin
       msg = {"Miscompare for ", uvm_object::m_sc.scope.get(), ": ", msg};
       uvm_report_info("MISCMP", msg, UVM_LOW);
    end
    miscompares = { miscompares, uvm_object::m_sc.scope.get(), ": ", msg, "\n" };
  endfunction
  ...
  function void print_rollup(uvm_object rhs, uvm_object lhs);
    string msg;
    if(uvm_object::m_sc.scope.depth() == 0) begin
  ...
  function void print_msg_object(uvm_object lhs, uvm_object rhs);
    ...
    if(result <= show_max) begin
      uvm_report_info("MISCMP", 
        ...
        uvm_object::m_sc.scope.get(), (lhs!=null ? lhs.get_inst_id() : 0), (rhs != null ? rhs.get_inst_id() : 0)), verbosity);
    end
    $swrite(miscompares, "%s%s: lhs = @%0d : rhs = @%0d",
        miscompares, uvm_object::m_sc.scope.get(), (lhs != null ? lhs.get_inst_id() : 0), (rhs != null ? rhs.get_inst_id() : 0));
  endfunction
