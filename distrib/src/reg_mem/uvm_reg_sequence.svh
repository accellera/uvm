...
//
// This class provides base functionality for both user-defined RegMem test
// sequences and "register translation sequences".
//
// - When used as a base for user-defined RegMem test sequences, this class
...
//   the RegMem model (held in the <regmem> property) or indirectly via the
...
class uvm_reg_sequence #(type BASE=uvm_sequence #(uvm_reg_bus_item)) extends BASE;
  ...
  // 
  // To maximize opportunities for reuse, user-defined RegMem sequences should
  ...
  //
  // This way, the RegMem sequence can be extended from ~any~ sequence, including
  ...
  // via <do_rw_access>. 
  //
  // User-defined RegMem test sequences must override body() and not call
  ...
  virtual task body();
    ...
    if (reg_seqr == null) begin
      `uvm_warning("REG_XLATE_NO_SEQR",
         {"Executing RegMem translation sequence on sequencer ",
    ...
    `uvm_info("REG_XLATE_SEQ_START",
       {"Starting RegMem translation sequence on sequencer ",
...
/*
  ...
  // to the originating RegMem sequence running on the upstream sequencer.
