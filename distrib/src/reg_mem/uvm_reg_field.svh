...
class uvm_reg_field extends uvm_object;
   ...
   constraint uvm_reg_field_valid {
      if (`UVM_REG_MEM_DATA_WIDTH > size) {
         value < (`UVM_REG_MEM_DATA_WIDTH'h1 << size);
      }
   }
   ...
   //
   extern virtual function bit          is_known_access(uvm_reg_mem_map map = null);
   ...
   /*local*/ extern virtual function uvm_reg_mem_data_t XpredictX (uvm_reg_mem_data_t  cur_val,
        	                                               uvm_reg_mem_data_t  wr_val,
                                                               uvm_reg_mem_map  map);

   /*local*/ extern virtual function void Xpredict_readX (uvm_reg_mem_data_t  value,
                                                          uvm_path_e path,
                                                          uvm_reg_mem_map  map);

   /*local*/ extern virtual function void Xpredict_writeX(uvm_reg_mem_data_t  value,
                                                          uvm_path_e path,
                                                          uvm_reg_mem_map  map);

   /*local*/ extern virtual function uvm_reg_mem_data_t XupdX();
...
function void uvm_reg_field::configure(uvm_reg            parent,
   ...
   if (size > `UVM_REG_MEM_DATA_WIDTH) begin
      `uvm_error("RegMem", $psprintf("Field \"%s\" cannot have more than %0d bits",
                                  this.get_full_name(), `UVM_REG_MEM_DATA_WIDTH))
      size = `UVM_REG_MEM_DATA_WIDTH;
   end
...
function bit uvm_reg_field::is_known_access(uvm_reg_mem_map map = null);
...
function string uvm_reg_field::get_access(uvm_reg_mem_map map = null);
   get_access = this.access;

   if (parent.get_n_maps() == 1 || map == uvm_reg_mem_map::backdoor)
     return get_access;
...
function void uvm_reg_field::set_attribute(string name,
                                         string value);
   if (name == "") begin
      `uvm_error("RegMem", {"Cannot set anonymous attribute \"\" in field '",
                         get_full_name(),"'"})
      return;
   end
   ...
   if (this.attributes.exists(name)) begin
      if (value != "") begin
         `uvm_warning("RegMem", {"Redefining attribute '",name,"' in field '",
...
function uvm_reg_mem_data_t uvm_reg_field::XpredictX (uvm_reg_mem_data_t cur_val,
                                                  uvm_reg_mem_data_t wr_val,
                                                  uvm_reg_mem_map    map);
   ...
   `uvm_fatal("RegMem", "uvm_reg_field::XpredictX(): Internal error");
   return 0;
...
function void uvm_reg_field::Xpredict_readX (uvm_reg_mem_data_t  value,
                                             uvm_path_e path,
                                             uvm_reg_mem_map     map);
...
function void uvm_reg_field::Xpredict_writeX (uvm_reg_mem_data_t  value,
                                              uvm_path_e path,
                                              uvm_reg_mem_map     map);
   if (value >> this.size) begin
      `uvm_warning("RegMem", $psprintf("Specified value (0x%h) greater than field \"%s\" size (%0d bits)",
...
function uvm_reg_mem_data_t  uvm_reg_field::XupdX();
...
function bit uvm_reg_field::predict(uvm_reg_mem_data_t  value,
                                    uvm_predict_e kind = UVM_PREDICT_DIRECT,
                                    uvm_path_e path = UVM_BFM,
                                    uvm_reg_mem_map     map = null,
                                    string          fname = "",
   ...
   if (this.parent.Xis_busyX && kind == UVM_PREDICT_DIRECT) begin
      `uvm_warning("RegMem", $psprintf("Trying to predict value of field \"%s\" while register \"%s\" is being accessed",
                                       this.get_name(),
...
function void uvm_reg_field::set(uvm_reg_mem_data_t  value,
                                 string          fname = "",
                                 int             lineno = 0);
   this.fname = fname;
   this.lineno = lineno;
...
function uvm_reg_mem_data_t  uvm_reg_field::get(string  fname = "",
                                            int     lineno = 0);
   this.fname = fname;
   this.lineno = lineno;
   get = this.desired;
...
task uvm_reg_field::write(output uvm_status_e  status,
                          ...
                          input  string             fname = "",
                          input  int                lineno = 0);
   uvm_reg_mem_data_t  tmp,msk,temp_data;
   uvm_reg_mem_map local_map, system_map;
   uvm_reg_mem_map_info map_info;

   bit [`UVM_REG_MEM_BYTENABLE_WIDTH-1:0] byte_en = '0;
   bit b_en[$];
   ...
   uvm_reg_mem_block  blk = this.parent.get_block();
   ...
   if (value >> this.size) begin
      `uvm_warning("RegMem", {"uvm_reg_field::write(): Value greater than field '",
                          get_full_name(),"'"})
...
`else	
	    ...
	    value = (value >> (n_access_extra)) & ((1<<this.size))-1;
	      ...
	      this.Xpredict_writeX(value, path, map);
   	    ...
   	    if(!adapter.supports_byte_enable) begin
               `uvm_warning("RegMem", $psprintf("Protocol does not support byte enabling to write field \"%s\". Writing complete register instead.", this.get_name()));
   	    end		
   	    if(!indv_acc) begin
               `uvm_warning("RegMem", $psprintf("Field \"%s\" is not individually accessible. Writing complete register instead.", this.get_name()));
   	    end		
            this.parent.XwriteX(status, tmp, path, map, parent, prior);
   	 end	
      end else begin
         `uvm_warning("RegMem", $psprintf("Individual field access not available for field \"%s\". Writing complete register instead.", this.get_name()));
         this.parent.XwriteX(status, tmp, path, map, parent, prior);
      end	
   end
   ...
   if(path == UVM_BACKDOOR) begin
      `uvm_warning("RegMem", $psprintf("Individual field access not available with BACKDOOR access for field \"%s\". Writing complete register instead.", this.get_name()));
      this.parent.XwriteX(status, tmp, path, map, parent, prior);
   end
`endif
...
`else
   ...
   if (path == UVM_BFM) begin
      ...
      if (this.individually_accessible) begin
	    ...
	    this.lineno = 0;
   	    ...
   	    if(!adapter.supports_byte_enable) begin
               `uvm_warning("RegMem", $psprintf("Protocol doesnot support byte enabling ....\n Reading complete register instead."));
   	    end		
   	    if((this.size%8)!=0) begin
               `uvm_warning("RegMem", $psprintf("Field \"%s\" is not byte aligned. Individual field access will not be available ...\nReading complete register instead.", this.get_name()));
   	    end		
            this.parent.read(status, reg_value, path, map, parent, prior, extension, fname, lineno);
            value = (reg_value >> this.lsb) & ((1<<this.size))-1;
   	 end	
      end else begin
         `uvm_warning("RegMem", $psprintf("Individual field access not available for field \"%s\". Reading complete register instead.", this.get_name()));
         this.parent.read(status, reg_value, path, map, parent, prior, extension, fname, lineno);
         value = (reg_value >> this.lsb) & ((1<<this.size))-1;
      end	
   end
   /// Individual field access not available for BACKDOOR access		
   if(path == UVM_BACKDOOR) begin
      `uvm_warning("RegMem", $psprintf("Individual field access not available with BACKDOOR access for field \"%s\". Reading complete register instead.", this.get_name()));
      this.parent.read(status, reg_value, path, map, parent, prior, extension, fname, lineno);
      value = (reg_value >> this.lsb) & ((1<<this.size))-1;
   end
`endif
...
task uvm_reg_field::poke(output uvm_status_e status,
   ...
   if (value >> this.size) begin
      `uvm_warning("RegMem", $psprintf("uvm_reg_field::poke(): Value greater than field \"%s\" size", this.get_name()));
      value &= value & ((1<<this.size)-1);
   end
   ...
   if (status != UVM_IS_OK && status != UVM_HAS_X) begin
      `uvm_error("RegMem", $psprintf("uvm_reg_field::poke(): Peeking register \"%s\" returned status %s", this.parent.get_full_name(), status.name()));
      this.parent.XatomicX(0);
...
function uvm_object uvm_reg_field::clone();
  `uvm_fatal("RegMem","RegMem field cannot be cloned")
  return null;
endfunction
...
function void uvm_reg_field::do_copy(uvm_object rhs);
  `uvm_warning("RegMem","RegMem field copy not yet implemented")
...
function bit uvm_reg_field::do_compare (uvm_object  rhs,
                                        uvm_comparer comparer);
  `uvm_warning("RegMem","RegMem field compare not yet implemented")
...
function void uvm_reg_field::do_pack (uvm_packer packer);
  `uvm_warning("RegMem","RegMem field cannot be packed")
endfunction
...
function void uvm_reg_field::do_unpack (uvm_packer packer);
  `uvm_warning("RegMem","RegMem field cannot be unpacked")
endfunction
