...
class uvm_reg_single_bit_bash_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      string mode[`UVM_REG_MEM_DATA_WIDTH];
      uvm_reg_mem_map maps[$];
      uvm_reg_mem_data_t  wo_mask;
      uvm_reg_mem_data_t  reset_val;
      int n_bits;
      ...
      foreach (maps[j]) begin
         uvm_status_e status;
         uvm_reg_mem_data_t  val, exp, v, other;
         int next_lsb;
         ...
         while (next_lsb < `UVM_REG_MEM_DATA_WIDTH) begin
            other[next_lsb] = 0;
            mode[next_lsb++] = "RO";
         end
         
         if (uvm_report_enabled(UVM_NONE,UVM_INFO,"RegMem"))
	 	`uvm_info("RegMem", $psprintf("Verifying bits in register %s in map \"%s\"...",
         ...
         if (val !== reset_val) begin
            `uvm_error("RegMem", $psprintf("Initial value of register \"%s\" ('h%h) not %s ('h%h)",
         ...
         */
         ...
         if (status != UVM_IS_OK) begin
            `uvm_error("RegMem", $psprintf("Status was %s when writing to register \"%s\" through map \"%s\".",
                                        status, rg.get_full_name(), maps[j].get_full_name()));
         end
         ...
         if (v !== exp) begin
            `uvm_error("RegMem", $psprintf("Writing 'h%h to register \"%s\" with initial value 'h%h yielded 'h%h instead of 'h%h",
   ...
   task bash_kth_bit(uvm_reg     rg,
      ...
      `uvm_info("RegMem", $psprintf("...Bashing %s bit #%0d", mode, k),UVM_MEDIUM);
      ...
      repeat (2) begin
         ...
         if (val !== exp) begin
            `uvm_error("RegMem", $psprintf("Writing a %b in bit #%0d of register \"%s\" with initial value 'h%h yielded 'h%h instead of 'h%h",
...
class uvm_reg_bit_bash_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      if (regmem.get_attribute("NO_REG_TESTS") == "") begin
        if (regmem.get_attribute("NO_BIT_BASH_TEST") == "") begin
           uvm_reg regs[$];
