...
class uvm_reg_shared_access_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      uvm_reg_mem_data_t  other_mask;
      uvm_reg_mem_data_t  wo_mask[$];
      ...
      foreach (maps[j]) begin
         ...
         `uvm_info("RegMem", $psprintf("Writing register %s via map \"%s\"...",
                                    rg.get_full_name(), maps[j].get_full_name), UVM_LOW);
         
         `uvm_info("RegMem", $psprintf("Writing 'h%h over 'h%h", v, prev),UVM_DEBUG);
         
         rg.write(status, v, UVM_BFM, maps[j], this);
         if (status != UVM_IS_OK) begin
            `uvm_error("RegMem", $psprintf("Status was %s when writing register \"%s\" through map \"%s\".",
                                        status.name(), rg.get_full_name(), maps[j].get_full_name()));
         end
         
         foreach (maps[k]) begin
            uvm_reg_mem_data_t  actual, exp;
            
            `uvm_info("RegMem", $psprintf("Reading register %s via map \"%s\"...",
            ...
            if (status != UVM_IS_OK) begin
               `uvm_error("RegMem", $psprintf("Status was %s when reading register \"%s\" through map \"%s\".",
                                           status.name(), rg.get_full_name(), maps[k].get_full_name()));
            end
            
            `uvm_info("RegMem", $psprintf("Read 'h%h, expecting 'h%h",
                                        actual, exp),UVM_DEBUG);
            
            if (actual !== exp) begin
               `uvm_error("RegMem", $psprintf("Register \"%s\" through map \"%s\" is 'h%h instead of 'h%h after writing 'h%h via map \"%s\" over 'h%h.",
...
class uvm_mem_shared_access_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      if (mem == null) begin
         `uvm_error("RegMem", "No memory specified to run sequence on");
         return;
      end
      ...
      if (mem.get_backdoor() == null) begin
         ...
         if (read_from < 0) begin
            `uvm_warning("RegMem", $psprintf("Memory \"%s\" cannot be read from any maps or backdoor. Shared access not verified.", mem.get_full_name()));
            return;
         end
      end
      ...
      foreach (maps[j]) begin
         ...
         `uvm_info("RegMem", $psprintf("Writing shared memory \"%s\" via map \"%s\".",
         ...
         for (int offset = 0; offset < mem.get_size(); offset++) begin
            ...
            if (mem.get_backdoor() != null) begin
               ...
               if (status != UVM_IS_OK) begin
                  `uvm_error("RegMem", $psprintf("Status was %s when reading initial value of \"%s\"[%0d] through backdoor.",
               ...
               if (status != UVM_IS_OK) begin
                  `uvm_error("RegMem", $psprintf("Status was %s when reading initial value of \"%s\"[%0d] through map \"%s\".",
            ...
            foreach (maps[k]) begin
               ...
               if (actual !== exp) begin
                  `uvm_error("RegMem", $psprintf("%s[%0d] through map \"%s\" is 'h%h instead of 'h%h after writing 'h%h via map \"%s\" over 'h%h.",
...
class uvm_reg_mem_shared_access_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      if (regmem.get_attribute("NO_REG_TESTS") == "" &&
