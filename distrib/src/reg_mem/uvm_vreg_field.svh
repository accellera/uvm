...
function void uvm_vreg_field::configure(uvm_vreg  parent,
   ...
   this.parent = parent;
   if (size == 0) begin
      `uvm_error("RegMem", $psprintf("Virtual field \"%s\" cannot have 0 bits", this.get_full_name()));
      size = 1;
   end
   if (size > `UVM_REG_MEM_DATA_WIDTH) begin
      `uvm_error("RegMem", $psprintf("Virtual field \"%s\" cannot have more than %0d bits",
                                     this.get_full_name(),
                                     `UVM_REG_MEM_DATA_WIDTH));
...
function string uvm_vreg_field::get_access(uvm_reg_mem_map map = null);
   if (this.parent.get_memory() == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg_field::get_rights() on unimplemented virtual field \"%s\"",
                                     this.get_full_name()));
      return "RW";
   end
...
task uvm_vreg_field::write(input  longint unsigned    idx,
   ...
   uvm_reg_mem_data_t  segval;
   uvm_reg_mem_addr_t  segoff;
   uvm_status_e st;
   ...
   if (mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg_field::write() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   if (value >> this.size) begin
      `uvm_warning("RegMem", $psprintf("Writing value 'h%h that is greater than field \"%s\" size (%0d bits)", value, this.get_full_name(), this.get_n_bits()));
   ...
   `uvm_info("RegMem", $psprintf("Wrote virtual field \"%s\"[%0d] via %s with: 'h%h",
...
task uvm_vreg_field::read(input longint unsigned     idx,
   ...
   mem = this.parent.get_memory();
   if (mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg_field::read() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Read virtual field \"%s\"[%0d] via %s: 'h%h",
                              this.get_full_name(), idx,
                              (path == UVM_BFM) ? "frontdoor" : "backdoor",
                              value),UVM_MEDIUM);
...
task uvm_vreg_field::poke(input  longint unsigned  idx,
   ...
   mem = this.parent.get_memory();
   if (mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg_field::poke() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Wrote virtual field \"%s\"[%0d] with: 'h%h",
...
task uvm_vreg_field::peek(input  longint unsigned  idx,
   ...
   mem = this.parent.get_memory();
   if (mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg_field::peek() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Peeked virtual field \"%s\"[%0d]: 'h%h", this.get_full_name(), idx, value),UVM_MEDIUM);
