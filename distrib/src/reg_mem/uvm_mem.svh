...
function uvm_status_e  uvm_mem::backdoor_read_func(
  ...
  foreach (paths[i]) begin
     ...
     if (val != data) begin
        `uvm_error("RegMem", $psprintf("Backdoor read of register %s with multiple HDL copies: values are not the same: %0h at path '%s', and %0h at path '%s'. Returning first value.",
                                    this.get_full_name(),
