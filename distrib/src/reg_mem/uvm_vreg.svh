...
class uvm_vreg extends uvm_object;
   ...
   local uvm_reg_mem_addr_t                offset;  // Start of vreg[0]
   ...
   //
   extern function void configure(uvm_reg_mem_block     parent,
                                  uvm_mem       mem    = null,
                                  longint unsigned  size   = 0,
                                  uvm_reg_mem_addr_t    offset = 0,
                                  int unsigned      incr   = 0);
   ...
   // that implements the virtual register array.
   //
   extern virtual function uvm_reg_mem_addr_t  get_offset_in_memory(longint unsigned idx);
   ...
   // address map, an error message is issued.
   //
   extern virtual function uvm_reg_mem_addr_t  get_address(longint unsigned idx,
...
function uvm_vreg::new(string       name,
   ...
   if (n_bits == 0) begin
      `uvm_error("RegMem", $psprintf("Virtual register \"%s\" cannot have 0 bits", this.get_full_name()));
      n_bits = 1;
   end
   if (n_bits > `UVM_REG_MEM_DATA_WIDTH) begin
      `uvm_error("RegMem", $psprintf("Virtual register \"%s\" cannot have more than %0d bits (%0d)", this.get_full_name(), `UVM_REG_MEM_DATA_WIDTH, n_bits));
...
function void uvm_vreg::configure(uvm_reg_mem_block      parent,
                                      uvm_mem        mem = null,
                                      longint unsigned   size = 0,
                                      uvm_reg_mem_addr_t     offset = 0,
                                      int unsigned       incr = 0);
...
function void uvm_vreg::add_field(uvm_vreg_field field);
   ...
   if (this.locked) begin
      `uvm_error("RegMem", "Cannot add virtual field to locked virtual register model");
      return;
   end

   if (field == null) `uvm_fatal("RegMem", "Attempting to register NULL virtual field");
   ...
   if (this.n_used_bits > this.n_bits) begin
      `uvm_error("RegMem", $psprintf("Virtual fields use more bits (%0d) than available in virtual register \"%s\" (%0d)",
   ...
   if (idx > 0) begin
      if (this.fields[idx-1].get_lsb_pos_in_register() +
         ...
         `uvm_error("RegMem", $psprintf("Field %s overlaps field %s in virtual register \"%s\"",
...
function void uvm_vreg::set_parent(uvm_reg_mem_block parent);
   this.parent = parent;
endfunction: set_parent

function uvm_reg_mem_block uvm_vreg::get_parent();
   get_parent = this.parent;
endfunction: get_parent

function uvm_reg_mem_block uvm_vreg::get_block();
...
function bit uvm_vreg::implement(longint unsigned n,
   ...
   if(n < 1)
   begin
     `uvm_error("RegMem", $psprintf("Attempting to implement virtual register \"%s\" with a subscript less than one doesn't make sense",this.get_full_name()));
      return 0;
   end

   if (mem == null) begin
      `uvm_error("RegMem", $psprintf("Attempting to implement virtual register \"%s\" using a NULL uvm_mem reference", this.get_full_name()));
      return 0;
   end

   if (this.is_static) begin
      `uvm_error("RegMem", $psprintf("Virtual register \"%s\" is static and cannot be dynamically implemented", this.get_full_name()));
      return 0;
   end

   if (mem.get_block() != this.parent) begin
      `uvm_error("RegMem", $psprintf("Attempting to implement virtual register \"%s\" on memory \"%s\" in a different block",
                                     this.get_full_name(),
                                     mem.get_full_name()));
      return 0;
   end
   ...
   begin
      ...
      if (min_incr > incr) begin
         `uvm_error("RegMem", $psprintf("Virtual register \"%s\" increment is too small (%0d): Each virtual register requires at least %0d locations in memory \"%s\".",
   ...
   if (offset + (n * incr) > mem.get_size()) begin
      `uvm_error("RegMem", $psprintf("Given Offset for Virtual register \"%s[%0d]\" is too big for memory %s@'h%0h", this.get_full_name(), n, mem.get_full_name(), offset));
      return 0;
   end
   ...
   if (mam_region == null) begin
      `uvm_error("RegMem", $psprintf("Could not allocate a memory region for virtual register \"%s\"", this.get_full_name()));
      return 0;
   end

   if (this.mem != null) begin
      `uvm_info("RegMem", $psprintf("Virtual register \"%s\" is being moved re-implemented from %s@'h%0h to %s@'h%0h",
                                 this.get_full_name(),
...
function uvm_mam_region uvm_vreg::allocate(longint unsigned n,
   ...
   if (mam == null) begin
      `uvm_error("RegMem", $psprintf("Attempting to implement virtual register \"%s\" using a NULL uvm_mam reference", this.get_full_name()));
      return null;
   end

   if (this.is_static) begin
      `uvm_error("RegMem", $psprintf("Virtual register \"%s\" is static and cannot be dynamically allocated", this.get_full_name()));
      return null;
   end

   mem = mam.get_memory();
   if (mem.get_block() != this.parent) begin
      `uvm_error("RegMem", $psprintf("Attempting to allocate virtual register \"%s\" on memory \"%s\" in a different block",
                                     this.get_full_name(),
                                     mem.get_full_name()));
      return null;
   end
   ...
   if (this.mem != null) begin
     `uvm_info("RegMem", $psprintf("Virtual register \"%s\" is being moved re-allocated from %s@'h%0h to %s@'h%0h",
                                this.get_full_name(),
                                this.mem.get_full_name(),
                                this.offset,
                                mem.get_full_name(),
...
function void uvm_vreg::release_region();
   if (this.is_static) begin
      `uvm_error("RegMem", $psprintf("Virtual register \"%s\" is static and cannot be dynamically released", this.get_full_name()));
      return;
   end
...
function uvm_reg_mem_addr_t  uvm_vreg::get_offset_in_memory(longint unsigned idx);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_offset_in_memory() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return 0;
   end
...
function uvm_reg_mem_addr_t  uvm_vreg::get_address(longint unsigned idx,
                                                   uvm_reg_mem_map map = null);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot get address of of unimplemented virtual register \"%s\".", this.get_full_name()));
      return 0;
   end
...
function int unsigned uvm_vreg::get_size();
   if (this.size == 0) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_size() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return 0;
   end
...
function int unsigned uvm_vreg::get_n_memlocs();
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_n_memlocs() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return 0;
   end
...
function int unsigned uvm_vreg::get_incr();
   if (this.incr == 0) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_incr() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return 0;
   end
...
function int uvm_vreg::get_n_maps();
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_n_maps() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return 0;
   end
...
function void uvm_vreg::get_maps(ref uvm_reg_mem_map maps[$]);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_maps() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return;
   end
...
function bit uvm_vreg::is_in_map(uvm_reg_mem_map map);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::is_in_map() on unimplemented virtual register \"%s\"",
                                  this.get_full_name()));
      return 0;
   end
...
function string uvm_vreg::get_access(uvm_reg_mem_map map = null);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_rights() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return "RW";
   end
...
function string uvm_vreg::get_rights(uvm_reg_mem_map map = null);
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot call uvm_vreg::get_rights() on unimplemented virtual register \"%s\"",
                                     this.get_full_name()));
      return "RW";
...
function uvm_vreg_field uvm_vreg::get_field_by_name(string name);
   ...
   `uvm_warning("RegMem", $psprintf("Unable to locate field \"%s\" in virtual register \"%s\".",
...
task uvm_vreg::write(input  longint unsigned   idx,
   ...
   this.lineno = lineno;
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot write to unimplemented virtual register \"%s\".", this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Wrote virtual register \"%s\"[%0d] via %s with: 'h%h",
...
task uvm_vreg::read(input  longint unsigned   idx,
   ...
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot read from unimplemented virtual register \"%s\".", this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Read virtual register \"%s\"[%0d] via %s: 'h%h",
                              this.get_full_name(), idx,
                              (path == UVM_BFM) ? "frontdoor" : "backdoor",
                              value),UVM_MEDIUM);
...
task uvm_vreg::poke(input longint unsigned   idx,
   ...
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot poke in unimplemented virtual register \"%s\".", this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Poked virtual register \"%s\"[%0d] with: 'h%h",
...
task uvm_vreg::peek(input longint unsigned   idx,
   ...
   if (this.mem == null) begin
      `uvm_error("RegMem", $psprintf("Cannot peek in from unimplemented virtual register \"%s\".", this.get_full_name()));
      status = UVM_NOT_OK;
      return;
   end
   ...
   `uvm_info("RegMem", $psprintf("Peeked virtual register \"%s\"[%0d]: 'h%h",
                              this.get_full_name(), idx, value),UVM_MEDIUM);
   ...
   if (this.size == 0)
     $sformat(convert2string, "%sunimplemented", convert2string);
      ...
      foreach (maps[i]) begin
        uvm_reg_mem_addr_t  addr0 = this.get_address(0, maps[i]);
