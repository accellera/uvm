...
function uvm_mam_region uvm_mam::reserve_region(bit [63:0]   start_offset,
   ...
   this.lineno = lineno;
   if (n_bytes == 0) begin
      `uvm_error("RegMem", "Cannot reserve 0 bytes");
      return null;
   end

   if (start_offset < this.cfg.start_offset) begin
      `uvm_error("RegMem", $psprintf("Cannot reserve before start of memory space: 'h%h < 'h%h",
   ...
   if (end_offset > this.cfg.end_offset) begin
      `uvm_error("RegMem", $psprintf("Cannot reserve past end of memory space: 'h%h > 'h%h",
                                     end_offset, this.cfg.end_offset));
      return null;
   end
    
    `uvm_info("RegMem",$psprintf("Attempting to reserve ['h%h:'h%h]...",start_offset, end_offset),UVM_MEDIUM)
   ...
   foreach (this.in_use[i]) begin
      if (start_offset <= this.in_use[i].get_end_offset() &&
         ...
         // Overlap!
         `uvm_error("RegMem", $psprintf("Cannot reserve ['h%h:'h%h] because it overlaps with %s",
...
function uvm_mam_region uvm_mam::request_region(int unsigned      n_bytes,
   ...
   if (!alloc.randomize()) begin
      `uvm_error("RegMem", "Unable to randomize policy");
      return null;
   end
...
function void uvm_mam::release_region(uvm_mam_region region);
   ...
   `uvm_error("RegMem", {"Attempting to release unallocated region\n",
...
task uvm_mam_region::write(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::write() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
...
task uvm_mam_region::read(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::read() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
...
task uvm_mam_region::burst_write(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::burst_write() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
...
task uvm_mam_region::burst_read(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::burst_read() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
...
task uvm_mam_region::poke(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::poke() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
...
task uvm_mam_region::peek(output uvm_status_e  status,
   ...
   if (mem == null) begin
      `uvm_error("RegMem", "Cannot use uvm_mam_region::peek() on a region that was allocated by a Memory Allocation Manager that was not associated with a uvm_mem instance");
      status = UVM_NOT_OK;
      return;
   end
