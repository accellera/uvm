...
class uvm_mem_single_walk_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      foreach (maps[j]) begin
         ...
         `uvm_info("RegMem", $psprintf("Walking memory %s in map \"%s\"...",
         ...
         for (int k = 0; k < mem.get_size(); k++) begin
            ...
            if (k > 0) begin
               ...
               if (status != UVM_IS_OK) begin
                  ...
                  if (val !== exp) begin
                     `uvm_error("RegMem", $psprintf("\"%s[%0d-1]\" read back as 'h%h instead of 'h%h.",
               ...
               if (status != UVM_IS_OK) begin
                  `uvm_error("RegMem", $psprintf("Status was %s when writing \"%s[%0d-1]\" through map \"%s\".",
                                              status.name(), mem.get_full_name(), k, maps[j].get_full_name()));
               end
            end
            ...
            if (k == mem.get_size() - 1) begin
               ...
               if (status != UVM_IS_OK) begin
                  ...
                  exp = ~(k) & ((1'b1<<n_bits)-1);
                  if (val !== exp) begin
                     `uvm_error("RegMem", $psprintf("\"%s[%0d]\" read back as 'h%h instead of 'h%h.",
                                                 mem.get_full_name(), k, val, exp));
...
class uvm_mem_walk_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      if (regmem.get_attribute("NO_REG_TESTS") == "") begin
        if (regmem.get_attribute("NO_MEM_WALK_TEST") == "") begin
           uvm_mem mems[$];
