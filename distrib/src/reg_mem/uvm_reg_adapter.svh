...
//
// <uvm_reg_predictor> : updates the RegMem mirror based on observed bus transactions
//
//
...
// is present, although a handle a <uvm_reg_mem_map> is provided in case the user
...
class uvm_reg_item extends uvm_sequence_item;
  ...
  // Variable: element
  //
  // A handle to the RegMem model element associated with this transaction.
  ...
  //
  rand uvm_reg_mem_data_logic_t value;
  ...
  //
  rand uvm_reg_mem_addr_t offset;
...
class uvm_reg_bus_item extends uvm_reg_item;
   ...
   rand int n_bits = `UVM_REG_MEM_DATA_WIDTH;
   ...
   // write/read.
   //
   rand uvm_reg_mem_byte_en_t byte_en = '1;            // if bus supports it
...
// Group: Example
//
// The following example illustrates how to implement a RegMem-BUS adapter class
...
//
// This adapter is used when running in a layering model that has RegMem sequences
// running on a separate, generic "upstream" sequencer connected to a
// "downstream" bus sequencer. In this case, the RegMem map associated with
// the downstream bus must be configured to start RegMem items on the generic
// upstream RegMem sequencer and performing no conversion.
//
...
class uvm_reg_predictor #(type BUSTYPE=int) extends uvm_component;
  ...
  // per logical RegMem transaction, which occurs when the bus width is smaller
