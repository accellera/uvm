...
class blk_R_test_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      data[7:0] = $urandom();
      regmem.R.write(status, data, .parent(this));
      regmem.R.mirror(status, UVM_CHECK, .parent(this));

      // Perform a random number of INC operations
      n = ($urandom() % 7) + 3;
      `uvm_info("blk_R_test_seq", $psprintf("Incrementing R %0d times...", n), UVM_NONE);
      repeat (n) begin
         regmem.CTL.write(status, reg_fld_B_CTL_CTL::INC, .parent(this));
         data++;
         void'(regmem.R.predict(data));
      end
      ...
      repeat (n) begin
         regmem.CTL.write(status, reg_fld_B_CTL_CTL::DEC, .parent(this));
         data--;
         void'(regmem.R.predict(data));
      end
      // Check the final value
      ...
      regmem.CTL.write(status, reg_fld_B_CTL_CTL::CLR, .parent(this));
      void'(regmem.R.predict(0));
      regmem.R.mirror(status, UVM_CHECK, .parent(this));
   endtask
