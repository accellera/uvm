...
class oc_ethernet_env extends uvm_env;
   ...
   virtual function void build();
      ...
      regmem = reg_block_oc_ethernet::type_id::create("regmem");
      host = wb_agent::type_id::create("host", this);

      regmem.build();

      regmem.set_hdl_path_root(hdl_root);
//      regmem.print();

      set_config_int("host.seqr", "count", 0);

      /*`ifdef VCS
      this.regmem.set_hdl_path_root("$root.tb_top.dut");
      `else
      this.regmem.set_hdl_path_root("tb_top.dut");
      `endif*/
   endfunction: build
   ...
   virtual function void connect();
      ...
      regmem.default_map.set_sequencer(seqr,reg2wsh);
