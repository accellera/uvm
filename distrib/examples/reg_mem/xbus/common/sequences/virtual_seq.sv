...
class virtual_seq extends uvm_reg_sequence;
  ...
  virtual task body();
    ...
    void'(regmem.xbus_rf.addr_reg.randomize());
    void'(regmem.xbus_rf.config_reg.randomize());

    regmem.xbus_rf.addr_reg.update(status, .parent(this));
    regmem.xbus_rf.config_reg.update(status, .parent(this));

    regmem.xbus_rf.config_reg.mirror(status, UVM_CHECK, .parent(this));
    regmem.xbus_rf.addr_reg.read(status, data, .parent(this));
