...
class mem_init_from_file_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      `uvm_info("mem_init_from_file_seq",
                {"Initializing ", regmem.xbus_rf.mem.get_full_name(),
      ...
      while ($fgets(line, fp)) begin
         ...
         regmem.xbus_rf.mem.write(status, idx, data, .parent(this));
      end
...
class mem_rand_init_seq extends uvm_reg_sequence;
   ...
   virtual task body();
      ...
      if (regmem == null && super.regmem != null)
        if(!$cast(regmem,super.regmem))
          `uvm_fatal("mem_init_from_file_seq",
              {"Must specify register model of type 'reg_sys_xa0'",
               "by assigning member 'regmem' before starting sequence"})

      `uvm_info("mem_rand_init_seq", {"Initializing ",
          regmem.xbus_rf.mem.get_full_name(), " with random data"}, UVM_LOW);
      
      idx = regmem.xbus_rf.mem.get_size();
      while (idx > 0) begin
         idx--;
         regmem.xbus_rf.mem.write(status, idx, $urandom(), .parent(this));
      end
   endtask
endclass
