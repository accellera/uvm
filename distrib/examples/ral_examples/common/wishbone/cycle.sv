...
class ral2wsh_adapter extends uvm_ral_adapter;
  ...
  virtual function uvm_sequence_item ral2bus(uvm_rw_access rw_access);
    ...
    cyc.m_kind = (rw_access.kind == uvm_ral::READ) ? wb_cycle::READ : wb_cycle::WRITE;
  ...
  virtual function void bus2ral(uvm_sequence_item bus_item, uvm_rw_access rw_access);
    ...
    rw_access.kind = cyc.m_kind ? uvm_ral::READ : uvm_ral::WRITE;
    rw_access.addr = cyc.m_addr;
    rw_access.data = cyc.m_data;
    // Send the result back to the RAL
    case (cyc.m_status)
       wb_cycle::ACK: rw_access.status = uvm_ral::IS_OK;
       wb_cycle::RTY: rw_access.status = uvm_ral::IS_OK;
       default      : rw_access.status = uvm_ral::ERROR;
    endcase
  endfunction
