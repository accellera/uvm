...
#ADAM too many </div>    $output .= '</div><!--Menu-->';
...
"\n\n\n <!--CONTENT index=$i -->\n" .
        '<div class="C' . NaturalDocs::Topics->NameOfType($parsedFile->[$i]->Type(), 0, 1) . '">'
          ...
          $sourceFileLink = '<a name="' . $anchor . '" href="' . $modSourceFile . '">' 
            . $modSourceFile . '</a>';

          $output .= '<a name="' . $anchor . '" href="' . $modSourceFile . '">' 
            . $self->StringToHTML( $parsedFile->[$i]->Title(), ADD_HIDDEN_BREAKS) . '</a>' ;
...
#ADAME - embed hierarchy in Summary instead
        ...
        #ADAME - prototype code used to be here; too late to stick in summary, so moved it North

        if (defined $parsedFile->[$i]->Body())
            {
            my $Body .= $self->NDMarkupToHTML( $sourceFile, $parsedFile->[$i]->Body(), $parsedFile->[$i]->Symbol(),
        ...
        my $lsep = $/;
        undef $/;
        open(FILE,"<$sourceFile") || die "Failed to open $sourceFile: $!\n";
        my $FILE = <FILE>;
        close FILE;
         $/ = $lsep;

        if ($FILE =~ m{//\s*\(inline source\)}i) {
          $output =~ s{\(inline source\)}{}i;
          $FILE =~ s{//\s+\(inline source\)}{}i;
          #$sourceFileLink =~ s/\.\.\///g;
          $output =~ s{</div></div></div>\n\n$}{\n\n};
          $sourceFileLink =~ s{\>(\.\.\/)+sv}{>../sv};
          $output .= "<p align=center>" . $sourceFileLink . '</p>';
          $output .= "</div>";
          $FILE =~ s{[^\n]?//-+\n[\s\S]*?\n//-+\n+}{}gm;
          $FILE =~ s{(//.*?)\n}{<b>$1</b>\n}gm;
          $FILE =~ s{\n *\n *\n *\n}{\n\n}g;
...
sub BuildSummary #(sourceFile, parsedFile, index, hierarhcy, prototype)  #ADAME: added hierarchy + prototype + multiSummary
    ...
    $output = $SummaryStart;
        ...
        while ($index < scalar @$parsedFile) {
            ...
            my $topic_type = NaturalDocs::Topics->NameOfType($topic->Type(), 0, 1);

# ADAME - make class type and description spans both columns
            ...
            $output .= "\n" .
             '<tr class="S' . ($index == 0 ? 'Main' : $topic_type)
                . ($indent ? ' SIndent' . $indent : '')
                . ($isMarked ? ' SMarked' : '') .'">'
                . '<td class=SEntry>';
            }
...
#ADAME added in_summary to choose different widths on prototype, summary vs. content section

sub BuildPrototype #(type, prototype, file, in_summary)
    {
    my ($self, $type, $prototype, $file, $in_summary) = @_;
...
#ADAME- reduce width threshold for using condensed format 
        my $limit = 80;
        if ($in_summary) {
          $limit = 55;
        }
...
#ADAME: symbol loses space between # in template classes somehow; causes lookup to fail
#$symbol =~ s/([^ ])#/$1 #/g;
...
#print "BuildClassHierarchy \@children = @children\n\n";
    @children = sort { ::StringCompare($a, $b) } @children;

    if (!scalar @parents && !scalar @children)
        {  return undef;  };

#ADAME- return undef if no parents; i.e. ignore children... we're not printing them
    if (!scalar @parents)
        {  return undef;  };
...
#ADAME: recurse to get complete inheritance tree
if (1) {
         my @list = ();
         my $item;
         #push @list, $parents[0];
         while (scalar @parents) {
           $item = $parents[0];
           #print "***** $symbol: parent[0]=$item\n";
           push @list, $item;
           @parents = NaturalDocs::ClassHierarchy->ParentsOf($item);
         }
         @parents = reverse @list;
}
...
print "Building Index type=$type\n\n";
$INDEX_TYPE=$type;
    # Build the content.
   ...
   for (my $i = 0; $i < 28; $i++)
    ...
    #ADAME - upped PAGESIZE_LIMIT to 250000
    use constant PAGESIZE_LIMIT => 250000;
    ...
    # If we're doing a top-level symbol entry...
        ...
        if (!$element->HasMultiplePackages())
            ...
            if (!$element->HasMultipleFiles())
                ...
                ## ADAME want index entries to appear the same regardless of whether one entry or multiple entries
                if (defined $element->Symbol() && !$element->Package())
                {
                my ($i, $s) =
                ...
                if (defined $packageText)
                    {
                    $indexHTML         .= ', <span class=IParent>' . $packageText . '</span>';
                    $searchResultsHTML .= ', <span class=IParent>' . $packageText . '</span>';
                    };

                 }
                 else
                 {
                 $indexHTML .= '<span class=ISymbol>' . $symbolText . '</span>' . '<div class=ISubIndex>';
                 $searchResultsHTML .= q{<a href="javascript:searchResults.Toggle('} . $searchResultID . q{')" class=ISymbol>}
                                         . $symbolText . '</a>' . '<div class=ISubIndex>';

                 my ($i, $s) = $self->BuildIndexElement($element, $cssID, $element->Symbol());
                 $indexHTML .= $i;
                 $searchResultsHTML .= $s;
                 $indexHTML .= '</div>';
                ...
                {
            #print "MultiFile !MultiPkg:  $symbolText\n";
                $indexHTML .= '<span class=ISymbol>' . $symbolText . '</span>';
                $searchResultsHTML .= q{<a href="javascript:searchResults.Toggle('} . $searchResultID . q{')" class=ISymbol>}
                                       . $symbolText . '</a>';
                ...
                if (defined $packageText)
                    {
                    $output .= ', <span class=IParent>' . $packageText . '</span>';
                    };

                $output .= '<div class=ISubIndex>'; 
                $indexHTML .= $output;
            ...
            foreach my $packageElement (@$packageElements)
                {
                #BuildIndexElement ($self, $element, $cssID, $symbol, $package, $hasPackage) = @_;
...
#ADAME- disable check for now-- don't use images in tooltips or summaries (yet), so this should be ok.
# what I want is for image location to be adjusted to the images/ dir.
            #if ($style == NDMARKUPTOHTML_GENERAL)
                #{
                
                #if ($text =~ /<img mode=\"inline\"/) {
            #print "\n\nHERE !!!!!!! $sourceFile $text \n\n";
                #}
                #if ($text =~ /<img mode=\"inline\" src=\"([^\"]*)\" original=\"([^\"]*)\">/) {
            #print "\n\nHERE !!!!!!! $sourceFile inline \n\n";
                #}
                # ADAME -rename target=* to src=*
                #$text =~ s{<img mode=\"inline\" target=\"([^\"]*)\" original=\"([^\"]*)\">}
                $text =~ s{<img mode=\"inline\" src=\"([^\"]*)\" original=\"([^\"]*)\">}
...
sub BuildImage #(sourceFile, mode, target, original)
 ...
 #no strict;
    #print Dumper($NaturalDocs::ImageReferenceTable);
