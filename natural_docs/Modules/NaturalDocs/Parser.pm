...
#ADAME - hack trying to debug- target= versus src=
        while ($body =~ /<img mode=\"[^\"]*\" src=\"([^\"]+)\" original=\"[^\"]*\">/g)
            {
...
sub CleanComment #(commentLines)
    ...
    while ($index < scalar @$commentLines)
        ...
        elsif ($line =~ /^([^a-zA-Z0-9_ ])\1{3,}$/ ||
                (length $line < 256 && $line =~ /^([^a-zA-Z0-9_ ])\1*([^a-zA-Z0-9_ ])\2{3,}([^a-zA-Z0-9_ ])\3*$/) )
            {
    ...
    while ($index < scalar @$commentLines)
        ...
        if ($commentLines->[$index] =~ /^ *([^a-zA-Z0-9_ ])\1{3,}$/ ||
            ( length $commentLines->[$index] < 256 &&
              $commentLines->[$index] =~ /^ *([^a-zA-Z0-9_ ])\1*([^a-zA-Z0-9_ ])\2{3,}([^a-zA-Z0-9_ ])\3*$/ ) )
        	{
        	if (!$inCodeSection)
        		{  $commentLines->[$index] = '';  }
        	}
	        ...
	        if ($leftSide == IS_UNIFORM)
	            ...
	            # symbol.
	            $commentLines->[$index] =~ s/^ *([^a-zA-Z0-9_ ])\1*//;
	            };
	        ...
	        if (($leftSide == IS_UNIFORM || $rightSide == IS_UNIFORM) && !$inCodeSection)
	            {
	            $commentLines->[$index] =~ s/^ *([^a-zA-Z0-9_ ])\1{3,}$//;
	            $commentLines->[$index] =~ s/^ *([^a-zA-Z0-9_ ])\1*([^a-zA-Z0-9_ ])\2{3,}([^a-zA-Z0-9_ ])\3*$//;
	            };
