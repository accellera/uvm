...
sub ParseFile #(sourceFile, topicsList)
    ...
    my $modified = 0;
    my $modifiedSourceFile = "$sourceFile.mod";
    if ($FILE =~ s{\n//\s*\(begin inline source\)(.*?)\n\/\/\s*\(end inline source\)}
      { &commentize($1); }egsi) {
      $modified = 1;
      my $modifiedSourceFile = "$sourceFile.mod";
      open(SOURCEFILEHANDLE, '>' . $modifiedSourceFile)
          or die "Couldn't open input file " . $modifiedSourceFile . "\n";
      print SOURCEFILEHANDLE $FILE;
...
# ADAME: attempt to extract class hierarchy
my $topic = $topicList->[-1];
my $proto = $topic->Prototype();
$proto =~ s/\n/ /g;
$proto =~ s/\s+/ /g;
my $parent;
if ($proto =~ /extends\s+(.+)$/) {
  $parent = $1;
  $parent =~ s/\s//g;
  if ($parent ne "ovm_void") {
  NaturalDocs::Parser->OnClassParent($topic->Package(), $parent, undef, undef, 0); #::RESOLVE_RELATIVE());
  }
}
